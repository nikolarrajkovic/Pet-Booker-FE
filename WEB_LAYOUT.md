...
| B19 | Admin lists (New Requests, Partners, Reviews) | Card queue, every item read up front and split into tabs on the client | One full-width row per item under a column header (`WebListRow`), paged from the server as you scroll, each tab in its own order — pending oldest first, decided newest first — with a `SortMenu` to change it |

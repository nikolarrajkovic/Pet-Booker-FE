...
| ChatThread (component) | `screens/messages-screen/components/ChatThread.tsx` | The thread itself — header, keyset-paged history, live hub subscription, typing, read ticks, composer (locked per `conversation.access`). `embedded` is the split view's frame: no back arrow, fills the pane instead of pinning to the viewport. The messages sit on the app's pet pattern (`PatternBackground onMobile`) — the one place the phone design wants the texture too, since a column of bubbles leaves most of the width empty either way. |
...
| AdminNewRequestsScreen | `screens/admin-new-requests-screen/containers/` | **API-wired** — `getServiceProviders()` in `useFocusEffect`; pending/approved/rejected tabs split on `approvalStatus`; approve → `approveServiceProvider()` (+ certs), reject → `declineServiceProvider()` (record kept, shows in Rejected tab); applicant email from `contactEmail` **Web design (2026-09):** a separate `*Web` component (the screen's default export only picks the design) pages from the server as the admin scrolls — `useModerationQueue` for the two moderation lists (per-tab `ApprovalStatus` + `Order`, pending oldest first / decided newest first, re-orderable via `SortMenu`, tab badges from `count*` one-row requests) — and draws full-width `WebListRow` rows with a column header instead of phone cards. The phone design is unchanged. |
...
| AdminPartnersScreen | `screens/admin-partners-screen/containers/` | Admin partner management list **Web design (2026-09):** `AdminPartnersWeb` pages approved providers from the server (newest first, re-orderable), searches by name server-side, and reads exact service / review counts off the provider row (`serviceCount`, `reviewCount`) instead of tallying the first 200 of each. Timeout/ban stay session-only overrides. The phone design is unchanged. |
| AdminReviewsScreen | `screens/admin-reviews-screen/containers/` | **API-wired** — review moderation. `getReviews({ perPage: 200 })` on focus (uses the embedded `user`/`serviceProvider` includes to render each card); Pending/Approved/Declined tabs split on `approvalStatus`. Approve → `approveReview()` (POST /admin/reviews/{id}/approve), decline → a reason-input modal → `declineReview(id, reason)`. Reached from the AdminDashboard "Reviews" quick action. **Web design (2026-09):** a separate `*Web` component (the screen's default export only picks the design) pages from the server as the admin scrolls — `useModerationQueue` for the two moderation lists (per-tab `ApprovalStatus` + `Order`, pending oldest first / decided newest first, re-orderable via `SortMenu`, tab badges from `count*` one-row requests) — and draws full-width `WebListRow` rows with a column header instead of phone cards. The phone design is unchanged. |

...
- **Fallback chain**: the amount's own currency → the user's `preferredCurrency` (mirrored into the module by `AuthContext` via `registerDisplayCurrency`, the same pattern as `registerApiLanguage`) → `DEFAULT_CURRENCY` (`RSD`). Changing it in Settings calls `refreshUser()` so mounted screens re-render. Note this falls back to the *display preference*, not `BASE_CURRENCY`: the lean `/api/home/*` rails omit `currency` on amounts the server already converted, so an RSD fallback would mislabel a converted EUR number. Use **`BASE_CURRENCY`** explicitly only where the UI means "what this will be charged as".
- **`ServiceDto.currency` is now on the wire (BACKEND_GAPS S8 resolved, verified live 2026-08-05)** — still read it via `serviceCurrency(svc)` (`services/services.ts`), which checks `svc.currency` then the `serviceProvider` embed and returns `undefined` so the preference fallback kicks in for the lean rails. Don't reach for `svc.currency` directly.

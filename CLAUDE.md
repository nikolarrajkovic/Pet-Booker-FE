...
- **Never write a currency symbol into JSX, a data constant, or a translation string.** A hardcoded `$` was wrong for every provider not pricing in dollars; a hardcoded `€` prefix was wrong for every currency whose symbol belongs *after* the number.
...
- **Display** → `formatMoney(amount, currency)`. Pass the currency **that came with the data** (`booking.priceCurrency`, a stats block's `currency`, `serviceCurrency(svc)`); omit it only for amounts the API doesn't stamp.
...
- **Fallback chain**: the amount's own currency → the user's `preferredCurrency` (mirrored into the module by `AuthContext` via `registerDisplayCurrency`, the same pattern as `registerApiLanguage`) → `DEFAULT_CURRENCY` (`RSD`). Changing it in Settings calls `refreshUser()` so mounted screens re-render.
- **`ServiceDto.currency` is now on the wire (BACKEND_GAPS S8 resolved, verified live 2026-08-05)** — still read it via `serviceCurrency(svc)` (`services/services.ts`), which checks `svc.currency` then the `serviceProvider` embed and returns `undefined` so the preference fallback kicks in for the lean `/api/home/*` rails. Don't reach for `svc.currency` directly.
- **The API converts money BOTH WAYS, and a write must DECLARE its currency.** Storage is always RSD, but a GET converts every amount into the caller's `preferredCurrency` and stamps the code it used (`ServiceReadDto.currency`, `BookingReadDto.priceCurrency`, a stats block's `currency`). A **write** converts *from* whatever `currency` the payload declares — and **omitting it means RSD**. So a form prefilled from a read holds display-currency numbers, and sending them back undeclared silently rescales them: verified live, a EUR viewer opening a service and saving it **unchanged** stored `0`. `services/currency.ts` exports **`declaredWriteCurrency(dataCurrency?)`** (the record's own code, else the display preference — both resolve from the same `preferredCurrency`, so they agree), and the four write DTOs that carry a `currency` stamp it **in the service layer, not at call sites**: `createService`/`updateService` (the root governs the whole aggregate — nested `additionalServices`/`pricingOptions`/`discounts` must NOT declare their own), `create/updateServicePricingOption`, and `create/updateServiceDiscount` (only a **Fixed** discount's amount is money; a Percent one is left alone). A forgotten declaration doesn't fail loudly — it corrupts prices — so keep it in the service function.

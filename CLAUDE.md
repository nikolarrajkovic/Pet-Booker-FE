...
- **Tab switches slide horizontally, in the direction the tab sits in the bar** (`navigation/tabTransition.ts` → `useTabSlideOptions()`, applied in `MainTabs`): Search → Home brings Home in from the left, Home → Search brings Search in from the right. Replaced the hard cut that stood from 2026-08-24 to 2026-09-14.
  ...
  - A scene is the window width on the phone design and the window **minus the sidebar** on the web one; getting that wrong shows up as the incoming screen arriving late. Pinned by `__tests__/responsive/tabTransition.test.ts`.
...
| `TabBar` | `state?`, `insets?` (both supplied by the navigator) | Bottom tab bar of the **mobile design** — destinations from `navigation/navItems.ts`, role-gated via `useAuth()`. **Renders `null` above 768px**, where `SideNav` is the navigation. Mounted once as the tab navigator's `tabBar`, not by each screen — see the App.tsx notes. |

...
services/           # All API calls and storage utilities
  ...
  service-schedules.ts  # ServiceScheduleDto CRUD + clearServiceSchedules — per-day working hours ride on the service write; this is only the "switch every day off" escape hatch
  ...
  service-discounts.ts  # ServiceDiscountDto, DiscountType (0=Percent,1=Fixed), getServiceDiscounts, create/update/deleteServiceDiscount, endOfDayIso. Server-enforced coherence (422 otherwise): Percent needs percentAmount in (0,100] (amount may repeat it); Fixed needs amount>0 and percentAmount=null; applyTo must be strictly AFTER applyFrom. `toWritableDiscount` enforces all three on the way out — write through these functions, never a hand-built body. Powers the Promotions "offer" create/edit flow.
  ...
  app-notifications.ts  # AppNotificationDto, NotificationType (ALL 20, 0–19), notificationTypeFromName, getAppNotifications, getInboxNotificationsPage, getUnreadNotificationCount, markNotificationRead, markAllNotificationsRead, notificationPayload/pushNotificationPayload, notificationBookingId (in-app notification INBOX)
  service-addons.ts # Helpers over a service's OPEN-ENDED `additionalServices` catalog (the three fixed Pickup/Drop-off/Special-Needs flags are gone): getEnabledServiceAddons, isPerDistance, requiredAddresses(For) (which address pickers an extra needs), addonPriceLabel ("5 € + 2 €/km", pre-distance), addonKey, toBookingSelection (→ ids-only booking payload). Deliberately holds NO pricing math — see booking-quote.ts.
  ...
  booking-quote.ts  # getBookingQuote(bookingBody) → POST /api/bookings/quote → BookingQuote. Prices a prospective booking WITHOUT creating it, through the same server code that will charge it. **The single source of every amount in the booking flow** — see Booking pricing below.
  distance.ts       # haversineKm — great-circle distance, DISPLAY ONLY ("has the device moved far enough to re-route the map?"). **Not a pricing input**; the surcharge formula that used to live here is gone.
  route-path.ts     # fetchRoutePath(from,to) → { coords, km, mins } — the drawable driving POLYLINE + ETA (OSRM demo server, straight-line fallback with mins:null, never throws). Powers the LiveSession inline directions map (both platforms). Display only — never a price input.
  home.ts           # getMostPopular / getOnSale / getRecentlyBooked / getNearMe — per-row Home endpoints (/api/home/*) → ServiceDto[] in the SAME read shape as /api/services (price/rating/imageUrl/photos/currency/discounts/schedules/options/add-ons; no reviews or upcomingBookings)
  stats.ts          # Dashboard aggregates (/api/stats/*): admin overview/banner/revenue-by-service-type, provider overview/earnings/latest-reviews/recent-activity, user overview/spending/recent-activity, platform (any signed-in account) + monthOverMonthChangePct. Server-side roll-ups — use INSTEAD of fetching lists and summing in JS.
...
hooks/          # Custom hooks
  ...
  useDebouncedValue.ts # A value that settles N ms after the last change — for filters that drive a request (a price slider emits a value per frame of the drag)
  useNearBottomLoader.ts # Loads the next page as the reader nears the bottom. **Native is a no-op** (the ScrollView's own onScroll covers it); the `.web.ts` variant listens to whichever ancestor actually scrolls, because `ScreenLayout` owns a single page-level scroll pane on web and flattens each screen's ScrollView into it — so the list's own `onScroll` never fires and the "infinite" scroll used to stop dead at page one. Verified by `visual/infinite-scroll.spec.ts` in a real browser; Jest cannot see it (no scrolling in a virtual tree)
...
navigation/
  ...
  tabTransition.ts    # useTabSlideOptions() — the horizontal tab slide (see Navigation)
assets/         # Images, fonts
...
### Request helpers — write every new service call with these
`apiJson` / `apiList` / `apiPage` / `apiVoid` take a **path** (not a URL) plus one options object, and fold the whole request dance — base URL, query string, `apiAuthFetch`, `response.ok`, `parseApiError`, pagination unwrap — into one call. The primitives below still exist and are what these are built from, but a service function should not be assembling them by hand.

- **`apiJson<T>(path, opts)`** — request returning a JSON object.
- **`apiList<T>(path, opts)`** — list endpoint → `T[]` (unwraps the pagination envelope).
- **`apiPage<T>(path, opts)`** — list endpoint → `PagedResult<T>` (for `usePagedList`).
- **`apiVoid(path, opts)`** — response body ignored (`DELETE`, `/admin/*` approve+decline, booking lifecycle POSTs). Never reads the body, so it is safe against `204 No Content`.
- **`apiRequest(path, opts)`** — the raw `Response`, for the rare case needing headers or a custom parse.
- Options: `{ method?, query?, body?, fallback, context, isPublic? }`. `fallback` + `context` are required — they are the message shown when the API says nothing useful, and the dev-console tag. `body` is JSON-stringified unless it is `FormData`. `isPublic: true` uses the unauthenticated `apiFetch` **and** sets the JSON headers itself (`apiFetch`, unlike `apiAuthFetch`, does not — a public POST without them 415s).
- **`buildQuery(params)`** — object → query string. Drops `undefined`/`null`/`''`; **keeps `false` and `0`**, which are real filter values here (`ApprovalStatus=0` is Pending, `IsRead=false` drives the unread badge). Keys are sent verbatim, and an unknown query param binds to nothing rather than erroring — check names against swagger.

```ts
export function getReviews(params?: GetReviewsParams): Promise<ReviewDto[]> {
  return apiList<ReviewDto>('/api/reviews', {
    query: { ServiceProviderId: params?.serviceProviderId, Page: params?.page ?? 1 },
...
### Primitives (what the helpers are built from)
...
- **`parseApiError(response, fallback, context?)`** — Extracts a human-readable message from a failed `Response`. Resolution order: the API's validation envelope (`{ error, details: [{ propertyName, errorMessage, errorCode }] }`) → ASP.NET ModelState (`{ errors: { Field: ["msg"] } }`) → `{ message }` → `{ detail }` → `{ title }` → `{ error }` → raw text → `fallback`. Pass a `context` tag (e.g. `'createPet'`) for the dev console log. **Use this in every `if (!response.ok)` block** instead of re-writing the parse logic.
  - Both validation shapes render **one rule per line**, with the field label (`Request.Phone` → "Phone", via the exported `humanizeFieldName`) prepended **only when the message doesn't already name the field** — the backend writes most rules in prose ("Phone must start with +381…"), so an unconditional prefix read "Phone: Phone must start with…", while terse FluentValidation messages ("must not be empty.") are meaningless without it.
  - **A JSON body it can't read never reaches the user** — the caller's `fallback` does. The old version returned the raw response text whenever no known key matched, so a 422 surfaced on screen as the literal `{"error":"Validation failed.","details":[…]}` blob. That is also why the `fallback` you pass should name the action ("Couldn't save your profile."), not repeat "Something went wrong".
...
- **Rule**: Never call `fetch()` directly. Prefer the request helpers above; drop to `apiFetch`/`apiAuthFetch` only where a call genuinely does not fit them (the token-extracting `/auth/login` + `/auth/refresh`, and the multipart uploads in `files.ts`).
...
If you are writing the pattern below by hand in a service, use `apiJson`/`apiList`/`apiPage`/`apiVoid` instead — it is what they do:
```ts
if (!response.ok) {
...
### `services/pets.ts`
- `getPets(ownerUserId)` → GET `/api/pets?OwnerUserId={id}` (auth) → `PetResponse[]`. **The filter is force-scoped server-side** (2026-08-06): a non-admin's `OwnerUserId` is replaced with their own id, so this always returns the caller's own pets. A provider reads a *booked* pet by id (allowed for the owner, an admin, or a provider with a booking for that pet — 404 otherwise), never by listing.
...
- Provider GET carries read-only `approvalStatus` (ApprovalStatus), `declineReason`, `isApproved` (legacy mirror), and writable `contactEmail` (the applicant's email). **`contactEmail` and `declineReason` come back `null` for anyone but an admin or the provider's owner** (2026-08-06, list and find-by-id alike) — the admin review screens are unaffected; don't build a user-facing feature on either.
...
- `pricing` contains: `{ basePrice, unit? (PricingUnit 0–3), isEscrowPercentEnabled, escrowPercent?, escrowAmount }`. **Add-on money is NOT here any more** — the old `pickupPrice`/`petReturnPrice`/`specialNeedsPrice` fields are gone; each optional extra is its own `additionalServices` entry (below).
- **`additionalServices`** — the service's **open-ended catalog of optional extras**, which replaced the three fixed add-ons (`details.is*Provided` flags + the `pricing.*Price` money). Each `AdditionalServiceDto` is `{ id?, serviceId?, name, description?, chargeType, price?, distancePrice?, distanceLeg?, isActive?, currency? }`. **`chargeType` decides which price field applies and the two must agree** (the API 422s a mismatch): `0 = Flat` uses `price`; `1 = PerDistance` needs a `distancePrice` (`LocationBasedPriceDto` — `{ baseFee, perKmFee, freeDistanceKm?, maxDistanceKm? }`) **and** a `distanceLeg` (`0 = Pickup`, `1 = DropOff`, `2 = RoundTrip`) naming which measured leg it bills. Writable **nested** on the service POST/PUT (upsert by id: known ids update in place, id-less rows are created, omitted rows are **removed**; omit the whole field to leave the catalog untouched) or standalone via `/api/service-additional-services` (not wired). Nested entries must NOT declare their own `currency` — the root's governs the aggregate. Read helpers live in `services/service-addons.ts`; the AddEditService editor maps onto it via `AdditionalServiceEntry` / `entryToAdditionalServices` (`my-services-screen/serviceModel.ts`). **Never reimplement the per-km formula from `distancePrice`** — ask the quote endpoint (see Booking pricing).
- `details` contains: `{ supportsLiveTracking, acceptedSpecies? (PetSpeciesType FLAGS, 63=All), minWeightKg?, maxWeightKg?, minDurationMinutes?, maxDurationMinutes?, leadTimeHours?, maxConcurrentBookings?, foodPricings? }`. **`supportsLiveTracking` is only legal on a Walker (1) or Transporter (5) service** — any other `type` 422s. The form has no toggle for it, so `uiToServiceDto` derives it from the type rather than round-tripping the stored value: otherwise re-typing a tracked Walker to Groomer failed on a setting the provider can't see. The add-on booleans that used to live here are gone with the fixed catalog. `acceptedSpecies`/`maxConcurrentBookings` are **non-nullable server-side** — omitting either on a PUT resets it to 0/None, so `uiToServiceDto(form, original)` round-trips them from the original DTO (defaults: 63 / 1 on create).
- `schedules` — per-day working-hours windows (`{ serviceId, day (0=Sun…6=Sat), from, to }` with `from`/`to` as `"HH:mm:ss"`), embedded on GET and **written nested on the service POST/PUT**: a non-empty array replaces the stored windows wholesale, an empty one leaves them untouched (the Photos convention). So the only case needing `/api/service-schedules` is clearing them all — `clearServiceSchedules` in `services/service-schedules.ts`. UI↔DTO translation is `serviceModel.ts` (`schedulesToWorkingHours`/`workingHoursToSchedules`).
- `pricingOptions` — duration/price variants (`ServicePricingOptionDto`: `{ id?, serviceId, name, description?, durationMinutes, price }`), embedded read-only on GET and **written nested on the service POST/PUT** (nullable array; a provided one is the desired full set upserted by id, `[]` clears every tier). The standalone `/api/service-pricing-options` CRUD (see `services/service-pricing-options.ts`) stays for editing one tier on its own. **Coexist rule (server-enforced): a service with ≥1 option requires every booking to send a `pricingOptionId`; an option-less service keeps classic free-range from/to booking.** `uiToServiceDto` writes `pricing.basePrice` = the cheapest tier so a card's "from" price is right even where only `price` is read. `effectiveOptionPrice(svc, option)` mirrors the server's discount math for display (percent → `price * (1 - amt/100)`, fixed → `max(0, price - amt)`, from `appliedDiscountType`/`appliedDiscountAmount`).
...
- `getServices(params?)` → GET `/api/services` (auth) → `ServiceDto[]`. Params: `serviceProviderId`, `name`, `type` (the service's own type), `providerType` (the OWNING PROVIDER's type — a different filter), `isActive`, `hasAdditionalServices`, `additionalServiceName`, `page`, `perPage`, plus the **browse filters** below. The fixed-add-on filters (`IsProvidingPickup`/`IsProvidingReturn`/`IsProvidingSpecialNeeds`) died with the fixed add-ons; the FE kept sending them until 2026-08-10, and since an unknown query param binds to nothing, a "filtered" search silently returned everything. **When adding a list filter, check it against `/swagger/v1/swagger.json` — a wrong name fails silently, it does not 400.**
- **Browse filters — applied by the SERVER, over the whole catalogue** (added 2026-09-15 with the one-per-line results list): `types[]` (multi-select counterpart of `type`), `minPrice`/`maxPrice` **+ `priceCurrency`**, `minRating`, `acceptedSpecies` (PetSpeciesType FLAGS OR-ed — matches a service accepting ANY of them), `additionalServiceNames[]` (the service must offer **all** of them), `onSaleOnly`, and `sortBy` (`ServiceSortBy`: Default/PriceAsc/PriceDesc/RatingDesc/MostReviewed/Newest/Popularity). **These exist because the results list pages itself as you scroll** — a `.filter()` over the loaded rows can only narrow the page that happened to arrive, so a narrow filter showed a handful of matches and the result count described that subset rather than the search. **Never filter a paged list in the client**; add the parameter to `ServiceSearch` on the backend instead.
  - `minPrice`/`maxPrice` bound the price the user is **shown** (base price after any live promotion), and **`priceCurrency` is not optional in practice**: reads are converted to the viewer's currency at the API edge, so a bound typed against those numbers is in that currency, and without the declaration the server compares it against stored RSD and matches almost nothing. `SearchScreen` sends `useCurrency().code`.
  - `minRating` excludes unrated services rather than treating them as 0 — "4 stars and up" is a claim about reviews.
  - Array params go out as **one repeated key each** (`Types=1&Types=2`), which `buildQuery` now does for any array value. Joining them (`Types=1,2`) binds to nothing and — since an unknown-shaped query param is ignored, not rejected — silently returns unfiltered results.
...
- `GET /api/services/{id}/availability?from=&to=` → `{ serviceId, days: [{ date, windows[] }] }` (wired — BookServiceScreen). `from`/`to` must be **date-only** strings (`2026-06-12`) — full ISO datetimes 400 — and the range is capped at **31 days** (422 naming `To`, as is `to < from`). An **unknown service id 404s** (it was a 422 until 2026-08-07). Readable by the **ServiceProvider group too**, so a partner screen can show its own free/busy. `windows` derives from the service's schedules, minus the provider's real bookings (`remainingCapacity`).
...
- **`clearServiceSchedules(existing)`** — deletes every one of a service's windows. The *only* schedule write not carried by the service POST/PUT: a non-empty nested `schedules` array replaces the stored windows wholesale, but an empty one means "leave untouched", so a provider switching every day off needs this. Do not reintroduce a per-day reconciler — it cost up to seven requests per save, each able to fail on its own.
- UI↔DTO translation lives in `screens/my-services-screen/serviceModel.ts`: `schedulesToWorkingHours(schedules)` (prefill, disabled-by-default Mon→Sun) and `workingHoursToSchedules(workingHours, serviceId)` (enabled days only, called by `uiToServiceDto`).
...
- **There is deliberately no bulk reconciler here.** Tiers are written nested on the service (`ServiceDto.pricingOptions` — the desired full set, upserted by id in the same request; `[]` reverts the service to classic free-range booking). Client-side diffing cost one request per changed tier and, because an option write declares no `currency` of its own, stored a provider's EUR prices as RSD; the nested write converts them with the aggregate's declared currency.
- UI↔DTO translation lives in `screens/my-services-screen/serviceModel.ts`: `pricingTiersToOptions(tiers, serviceId)` (only tiers with a resolvable duration + parseable price become options; the duration label is the option `name`, called by `uiToServiceDto`) and the shared duration label↔minutes map (`DURATION_OPTION_LABELS`, `minutesToDurationLabel`, `durationLabelToMinutes` — unmapped minutes decode as `"{n} min"`).
...
- **`comment` is REQUIRED and must be ≥ 10 characters** (verified live 2026-08-10: absent/null/empty → 422 "An explanation of your experience is required."; 1–9 chars → 422 "The explanation must be at least 10 characters long."). `ReviewModal` enforces this (`MIN_COMMENT_LENGTH`) — it used to advertise the comment as optional, so the most natural review (stars only) was always rejected with a message the user had no field to fix.
...
- **A booking's money is server-owned — `basePrice`/`discountAmount` are IGNORED on write** (2026-08-06). The server derives both from the booked service (or the chosen pricing option) on create, keeps the row's values on update (preserving a provider's adjust-price) and re-derives only if the booking is re-pointed at another service; `totalPrice` is recomputed from them plus the add-on lines. `POST /bookings/{id}/adjust-price` is the only override, and it's provider-only. They stay on `CreateBookingInput` because the quote returns them and the read carries them back — but never treat a locally-computed price as authoritative. (Before the fix the client's numbers were trusted whenever the service had no live promotion: a 100 RSD service could be booked for 1, and omitting the field stored 0.)
- **`priceCurrency` is server-stamped** from the booked service's provider (`ServiceProviderDto.currency`, default EUR); a client-sent value is ignored, so `createBooking` sends `null`. Render every booking amount through **`formatMoney(amount, currency)`** (see **Money & Currency** below; still re-exported from `services/bookings.ts`). **Never hardcode a `$`.**
- **Payload rounding:** every outgoing booking payload (`createBooking` POST + `setBookingStatus` PUT) is passed through `round2Payload()` before `JSON.stringify` — it deep-rounds **all** numbers to ≤2 decimals (`basePrice`/`discountAmount`/`totalPrice`, etc.) **except `latitude`/`longitude`, which keep full precision** (2-decimal coords would shift a point ~1km). Integers (ids/enums/flags) round to themselves. So no price/distance is ever sent with >2 decimals, while address coordinates stay exact.
- **Add-on selection is an ARRAY OF IDS — no flags, no amounts, no distances.** The write DTO carries `additionalServices: { additionalServiceId }[]`, each naming one entry in the booked service's catalog (build it with `toBookingSelection()`); the old `includePickup`/`includePetReturn`/`includeSpecialNeeds` booleans and the single `distanceKm` are gone. On update the array is the **desired full set**, so a deselected extra loses its line entirely, and `toWritableBooking` round-trips it so a status/cancel PUT doesn't wipe the bill. On READ the same field comes back **enriched** as `BookingAdditionalServiceReadDto[]` — `{ additionalServiceId?, name, chargeType, distanceLeg?, price, baseFee?, perKmFee?, distanceKm? }`, frozen at the moment the booking was priced. A per-distance line therefore carries the fees **and its own leg's distance** behind its amount, so a screen can render "200 + 8.4 km × 50 = 620" without recomputing anything (and keeps rendering it correctly even if the provider later re-prices the catalog entry). `addOnsTotal` / `depositAmount` / `totalPrice` are read-only server output — a client-sent `totalPrice` is recomputed. Addresses persist and round-trip (BACKEND_GAPS B2 resolved 2026-06-22): inline `location.pickupAddress`/`leaveOverAddress` are saved and returned under `location.*` on GET, and `withResolvedAddresses()` mirrors them back top-level for consumers.
- **Booking pricing is SERVER-SIDE, end to end — the client measures nothing (2026-08).** Both trip legs are routed by the backend (`TripDistanceResolver` → Google Routes API v2 → OSRM → great-circle ladder) from the addresses under `location`, and each leg is measured **in the direction it's driven** (provider → customer for the pickup, customer → provider for the return), so a booking with different pickup and drop-off addresses bills two genuinely different distances. The formula (`baseFee + perKmFee × max(0, min(km, maxDistanceKm) − freeDistanceKm)`; `maxDistanceKm` **caps billable km, it does not reject the booking**) is applied there and **must not be mirrored here**.
  - **To show a price before the booking exists, call `getBookingQuote()` (`POST /api/bookings/quote`, `services/booking-quote.ts`)** with the same body `createBooking` will send. It runs the identical pricing code, persists nothing, and returns `{ bookingFrom, bookingTo, basePrice, discountAmount, pickupDistanceKm, leaveOverDistanceKm, additionalServices[], addOnsTotal, totalPrice, depositAmount, priceCurrency }`. `bookingTo` is echoed because a `pricingOptionId` makes the server derive it. A null `pickupDistanceKm`/`leaveOverDistanceKm` means that leg wasn't measurable (no address or no resolvable coords) — the cue that a per-distance extra is showing base fee only.
  - The quote is deliberately **permissive** where a create is strict: an extra the service doesn't offer is skipped rather than rejected, and schedule/capacity rules don't run (the user is still choosing). **A successful quote promises the price, not that the create will succeed.**
  - **Never send a distance.** `distanceKm` isn't on the write DTO, and the server discards a non-admin's override anyway: declaring the distance is declaring the price (0 km on a "200 + 50/km" pickup turns 1200 into 200).
  - Why it moved: the client used to geocode the service address, route to the picked point with Google Directions, then apply its own copy of the surcharge formula. Directions only worked in the web bundle, so **native silently priced on straight-line distance** — a different price per platform, and neither guaranteed to match the charge. `services/directions.ts` and the surcharge helpers in `services/distance.ts` were deleted with it.
...
- **Platform**: `getPlatformStats()` → `{ currency, activePartners, averageProviderRating, totalReviews, averageMonthlyPartnerEarnings, earningPartners, earningsWindowMonths }`. The **only stats route a plain customer can read** (User + ServiceProvider groups, added 2026-08-12 for the BecomePartner pitch), which is why it carries aggregates only — no revenue total, booking volume, or per-account figure. **Guard each figure on its backing count**: `averageProviderRating` is 0 when `totalReviews` is 0 and `averageMonthlyPartnerEarnings` is 0 when `earningPartners` is 0, and both zeros mean "no data yet", not a measurement. Earnings = each *earning* partner's captured payments over the trailing `earningsWindowMonths` ÷ that window, averaged across them (idle partners are excluded so a new signup can't drag the figure to zero).
...
### `services/messages.ts` + `services/message-hub.ts`
Direct customer↔provider messaging, **wired 2026-08-12** against `/api/chat/*` + `/hubs/chat`. Note the `chat` segment: the routes are NOT `/api/conversations` (an earlier spec in this repo assumed that — they never existed under that path).
- **A conversation is the customer↔provider pair**, one row per pair forever (get-or-create), with `serviceId` recording which service the customer opened it from — subtitle context, not a scope, refreshed when they open the chat from a different service. Rebooking reuses the same thread with its whole history.
- **`GET /api/chat/conversations` takes NO owner parameter.** The server scopes the inbox to the session and merges both sides for a partner (a customer of other providers as well as a provider). It also resolves `counterpartName`/`counterpartAvatarUrl` per row, so one shape serves both inboxes. Rows arrive already ordered by activity — don't re-sort.
- **Bubble side comes from `sender` (0=customer, 1=provider) compared with the conversation's `viewer`, NEVER from `senderUserId`.** A company-managed ProviderProfile account has no `Domain.User`, so `senderUserId` is null on its messages and an id comparison renders every one of them as the customer's. `senderUserId` is audit data.
- **History is keyset-paged**, not page-numbered: `GET .../messages?before={oldestIdYouHold}&limit=` → `{ items (oldest→newest), nextBefore, hasMore }`. A message arriving mid-scroll would shift an offset page boundary and repeat or skip a line.
- **Whether the composer is open is the server's call, in `conversation.access`** (`ChatAccessDto`): any live booking → open and unbounded; 14 days after a service (extended by leaving a review); 3 days after a cancellation; otherwise a **bounded pre-booking enquiry** against an approved provider — `remainingEnquiryMessages` (default 3) until they reply, and their reply resets it. **Never re-derive the rule client-side** — render `access` and let a 401 be impossible. A closed thread is read-only, never hidden (`ComposerLockedNotice`); reasons map to `messages.locked*` strings.
- **Two hub events per message, and the difference matters**: `ChatMessageReceived` goes to the thread group, which a screen must join via `SubscribeToConversation` (server-authorized); `ChatInboxUpdated` goes to the caller's identity group, joined automatically from the JWT on connect, and is **recipient-only** — that is the one that drives the badge, so a user's own sends can never inflate it. Plus `ChatMessagesRead` (`{ conversationId, reader, upToMessageId, readAt }` — `reader` is a participant enum, not a user id) and `ChatTypingChanged`.
- `sentAt`/`readAt` are **true UTC instants** — read with `new Date()`, never `parseBookingDate`.
- **The thread on screen is claimed in `services/active-conversation`**, and *both* hubs honour it: the chat hub's toast and the notification hub's `NewChatMessage` toast skip it (the backend files one every time a thread goes 0 → 1 unread, which an open thread does on every arrival), and the split view's inbox row for it never shows an unread pill (`useConversationsInbox(openConversationId)`).
- **Missed pushes are not replayed.** After a reconnect (or a first connect that had to retry) `MessagesContext.subscribeToReconnect` fires: the open thread re-reads its newest page and merges it by id, and the inbox reloads.
- Outgoing ticks: single grey = sent, double **blue** = read (`MessageBubble`) — not the brand green, which is the bubble's own colour.
- **In a browser, Enter sends and Shift+Enter adds a line** (`MessageComposer.onKeyPress`, web only; an IME still composing keeps Enter). A multiline field never fires `onSubmitEditing` on Enter there, so without it a message could only be sent with the mouse. Phones keep their keyboard: return adds a line, the button sends.
...
- **The web design draws the open thread with the inbox beside it on the right** (`MessagesSplitView`) — the right, because the sidebar already runs down the left edge — with the most recent thread preselected; the phone pushes the thread over the list. Both message routes render it, so a thread reached from a notification looks like one reached by clicking. The thread itself is `components/ChatThread` in every case — the screens only pick the frame (`embedded`). See the Screens Reference rows below.
...
- `usePushNotifications(navReady)` is mounted **once in App.tsx**, not in a screen: a cold start from a notification arrives before any screen exists, and `getLastNotificationResponseAsync()` is the only way to see it (it is not delivered as an event).
...
- A **tap** goes through the shared route table in `navigation/notificationRoute.ts` (see below), and **marks its notification read** — the push data carries `notificationId`, so a user who deals with everything from the lock screen doesn't come back to a badge with nothing new behind it. Foreground arrivals are deliberately *not* routed — the badge and the live SignalR push already update the UI, and yanking someone out of what they're doing would be hostile.
- **The push toggle answers to the DEVICE, not just the account.** `UserNotificationSettings.pushEnabled` is a preference; whether anything arrives is the OS's call, and the two used to be able to disagree silently — a phone with notifications blocked showed push as ON and dropped every notification. `getPushPermission` / `requestPushPermission` / `openDeviceNotificationSettings` expose the OS side, `hooks/usePushPermission.ts` holds it as screen state, and NotificationSettingsScreen renders push as on only when **both** agree. Turning it on asks the OS first and records nothing if refused; turning it off retires this handset's token (`unregisterPushDevice`) as well as clearing the preference.
- **`usePushPermission` re-reads on `AppState` 'active'.** Every route out of a permanent denial leaves the app (Open Settings → flip → come back) and nothing tells the app it happened, so without that listener the screen keeps showing "blocked" over a permission that is now granted.
- Requires an **EAS `projectId`** for a token (SDK 49+); without one registration logs and skips.

### `navigation/notificationRoute.ts` — where a notification leads
One routing table for **both** taps a notification can get: the in-app toast/inbox row and the OS push. They are different transports — a stored notification has a numeric `type` and a `dataJson` string, a push has a flat data bag whose `type` is the enum's **NAME** (`AppNotificationDispatcher.BuildPushData` sends `type.ToString()` plus `notificationId`) — so both are normalized into a `NotificationPayload` (`services/app-notifications.ts`) and routed by `routeForPayload`. **Never route on the ids alone**: that is what let the push and the in-app tap disagree, sending a "service completed" push to the booking while the same row in the inbox offered the review modal.
- A route is a **list** of screens, so the destination gets its natural parent behind it — a thread gets `Messages`, every provider-facing screen gets the Partner tab.
- Per type: request → `NewRequests` (the screen with Accept/Decline, not the read-only recap); live tracking → `LiveSession`; service completed → the inbox, which auto-opens `ReviewModal` (the inbox passes `{ reviewHandled: true }` so an already-reviewed booking falls through to `BookingDetails` instead of a tap that does nothing); review approved/received → `ServiceDetail` for the service the payload names; review declined → the feed; **an approved partner application → `PartnerWelcome`**, the celebration + tour, which is the one partner decision with a screen of its own; the remaining provider/certificate decisions → the Partner hub, having none; message → `Chat`; **everything else → `BookingDetails`**, which is also the fallback for a type this build predates, so a notification added to the backend later still opens its booking rather than being swallowed.
- `NotificationType` keys are the **backend enum's member names** — they are wire names, not labels, because `notificationTypeFromName` resolves a pushed type by looking one up. Renaming a key silently breaks push routing.
- Per-type icons live in `screens/notifications-screen/components/NotificationItem.tsx`, keyed by the same names as `Record<keyof typeof NotificationType, …>` — so **adding a type to the enum without giving it a look is a compile error**, rather than a row that quietly renders as a generic bell.
...
- **Paging is bounded — `PerPage` caps at 200** (verified live 2026-08-10: `PerPage=500` answers `itemsPerPage: 200`). `Paginate=false` drops the *wrapper*, not the bound — it returns one unwrapped page of ≤200 rows. Non-positive `Page`/`PerPage` are normalised (`Page=0&PerPage=-5` → page 1 × 10), not rejected, so they no longer 500. **Raising `PerPage` is never the way to see a long list** — page it (`extractPage`/`PagedResult` in `services/http.ts`), or use a `/api/stats/*` aggregate.
...
- **Service GET returns more than the write DTO**: also includes `rating`, `totalRatingNumber`, `price` (effective price after discount — prefer over `pricing.basePrice` for display), `about` (read-only mirror of `description`), `imageUrl`, `basicServiceName`, `appliedDiscountType`, `appliedDiscountAmount`, `discounts[]`, `schedules[]`, plus the embedded includes **`reviews[]`/`reviewCount`** (service-level, ALL moderation statuses — public screens must filter `approvalStatus === Approved`) and `upcomingBookings[]`. These extra fields are typed as optional on `ServiceDto`. The **Home rails return this same read shape** (verified live 2026-08-10) minus those two loader-populated includes — until 2026-08-07 they served the WRITE shape, so every card field came back missing.
- **A service read carries NO provider record** — only `serviceProviderId` (verified live 2026-08-10; there is no `serviceProvider` embed, unlike the booking and review reads). A screen needing the provider's name/photo/address/verified badge calls `getServiceProvider(serviceProviderId)` as a second, fail-soft request — see ServiceDetailScreen. **Find-by-id on a service now 404s when its provider isn't Approved** (it used to be readable one id at a time while search and the rails hid it).
...
- **Creating a booking REQUIRES a valid `paymentMethodId`** referencing an existing PaymentMethod for the user. Posting with `null`/`0`/missing → 422 ("must not be empty" + "must reference a real one"). So the booking flow has a hard prerequisite: the user must have a saved payment method. Extras are picked by sending their catalog ids in `additionalServices` (see the bookings.ts section) — the server prices each one and freezes it as a bill line. `location` (write DTO: `BookingLocationDto`) is sent **inline** under `location.pickupAddress` / `location.leaveOverAddress`, and the create path now **persists it** (verified live 2026-06-22): the POST creates a real address row and the booking GET returns it under `location.pickupAddress` / `location.leaveOverAddress` (BACKEND_GAPS B2 resolved). (`/api/addresses` still exists standalone but requires a non-empty `state`; the booking flow uses the inline shape.)
- **Creating a pet REQUIRES at least one photo** — `createPet()`/`AddPetScreen` must enforce ≥1 photo before POST (currently it does not — a photoless pet 422s). The message used to name a binding wrapper (`'Request Photos' must not be empty`); since 2026-08-07 validation errors carry a **humanized, localized field label** (and the Serbian is Latin, not Cyrillic). `details[].propertyName`/`errorCode` are unchanged, so anything keying off those still works — surface the message via `parseApiError` rather than matching on its text.
- **Reads are scoped to the caller** (2026-08-06). `GET /api/pets` is **force-scoped**: a non-admin's `OwnerUserId` is overwritten with their own id, so it only ever lists your own pets (a provider reads a *booked* pet by id instead — find-by-id allows admin / the owner / a provider with a booking for that pet, and 404s otherwise). On providers, **`contactEmail` and `declineReason` are owner/admin only** on both the list and find-by-id — the admin screens still get them, a browsing user gets `null`. Certificates are public only once their provider is Approved.
...
- ~~`POST /api/bookings/quote`~~ — **now wired 2026-08** via `services/booking-quote.ts` → BookServiceScreen. Prices a prospective booking through the create's own pricing code without persisting anything; it is what replaced the client-side distance measurement + surcharge formula (see Booking pricing under `services/bookings.ts`).
...
- **Payments (replaced the old `checkout-session` endpoints, verified 2026-06-22):** `GET /payments/bookings/{bookingId}/summary` → `{ bookingId, currency, totalPrice, depositAmount, amountPaid, balanceDue, payments[] }` (live ledger), `POST /payments/bookings/{bookingId}/pay` (`{ phase: 0|1, paymentMethodId }`), and `GET /api/booking-payments` (records, read-only). Not yet wired — stored-card management still doesn't exist (AU2). **Rules to build against when it is wired (2026-08-07):** a booking only takes money between the provider's confirmation and the end of the service — paying a *requested* booking (the provider can still decline or re-price it) or a cancelled one is rejected, and there is no refund path; a named `paymentMethodId` must belong to the booking's customer; **paying the deposit no longer blocks cancelling** (a customer may still call off a not-yet-started booking); and the **provider may read the summary** (is the deposit in?) though only the customer/admin may pay.
- `POST /bookings/{id}/adjust-price` (`{ basePrice?, discountAmount?, additionalServices?, currency? }` — per-line overrides by bill-line id) — provider re-prices a booking, returns the recomputed booking. Not wired. `POST /api/service-additional-services` CRUD — the standalone add-on catalog endpoints; not wired either (the FE manages extras nested on the service write). `GET /api/bookings/{bookingId}/live-location` — GPS trail (`{ sessionId, sessionStatus, isActive, latest, trail[] }`) — **wired 2026-07** (`services/live-location.ts`, LiveSession live map + SignalR `/hubs/location`).
...
- ~~`GET /api/app-notifications` (+ mark-as-read PUT)~~ — **now wired** (in-app notification inbox) via `services/app-notifications.ts` → NotificationsScreen. Read-only on the collection (no POST in swagger — notifications are created server-side by booking/account events); `GET /{id}`, `PUT /{id}` (write DTO is only `{ id, isRead }` — server stamps `readAt`), `DELETE /{id}`. List filters: `UserId`, `IsRead`, `Type`, `Page`/`PerPage`/`Paginate`. `NotificationType` (**0..19, complete as of 2026-09** — 16 BookingUpdated, 17 ReviewReceived, 18 ReviewApproved, 19 PaymentDue were the last additions) is a swagger enum NOT exposed via `/enums` — mirrored in `app-notifications.ts`, where the keys are the backend enum's own member names because a push carries `type.ToString()`. **The server keeps chat out of the feed itself** (`NotificationFeed`/`SearchAppNotificationsQuery`: any search that does not name a `Type` filters the excluded types out, and naming one opts back in), so the client must NOT subtract them again — doing so left the bell reading low by however many unread messages the user had. The client-side `isInboxNotification` filter stays only for the row the server never sees: a live SignalR push, which arrives whatever its type. `GET/POST /api/user-push-devices` (push token registration) is wired — see `services/push-registration.ts` below. **Real-time pushes (wired 2026-07):** `context/NotificationsContext.tsx` holds one app-wide SignalR connection to `/hubs/notifications` (`services/notification-hub.ts`; the server auto-joins the caller's `user:{id}` group from the JWT — no subscribe call). Every persisted notification also arrives live as `NotificationReceived` (same `AppNotificationDto` shape): the provider bumps the bell badge (`useNotifications().unreadCount`, consumed by HomeScreen), shows a toast with the title, and fans the payload out to subscribers — NotificationsScreen prepends it to the inbox without refetching. REST stays the source of truth (count re-seeded on login/reconnect/focus). Shared hub plumbing lives in `services/hub-connection.ts` (token refresh via `accessTokenFactory`, **`startHubConnection`** + **`hubRetryPolicy`** — a failed *first* connect is retried and a reconnect never gives up; SignalR's own reconnect covers neither, and either gap left a tab silently not-live until reload — and **`withCredentials: false`** — the signalr client's default credentialed fetch is rejected by browsers against the API's wildcard CORS).
...
- ~~`GET /api/stats/*` (10 endpoints)~~ — **now wired 2026-07-27** via `services/stats.ts` → AdminDashboard + PartnerHub. The **user** group (`/stats/user/overview|spending|recent-activity`) is wrapped but has no consumer screen yet — an obvious fit if Profile ever grows a stats block. `GET /api/stats/platform` was **added to the backend 2026-08-12** (new: `PlatformStatsDto` + `GetPlatformStatsQuery`, `[UserGroupAuth(User, ServiceProvider)]`) for the BecomePartner card — the admin group is the only other source of platform figures and a prospective partner can't read it.
- `POST /auth/provider-profiles/register` (`{ providerProfileId, password }`) — create a login account from a provider profile.
- **Still unwired** (audited against swagger 2026-07-27, 100 paths): `/api/user-pets` (user↔pet join rows, incl. `isPrimaryOwner` — pets are read via `/api/pets?OwnerUserId=` today), `/api/certificates` + `/api/photos` + `/api/provider-profiles` + `/api/booking-statuses` (managed through their parent entities instead), `/api/addresses` GET/PUT/DELETE (only POST is used), `/admin/accounts`, `/admin/groups`, `/admin/roles`, `/admin/users`, `/admin/location-sessions/{id}/retain`, `GET /files` + `POST /files/cleanup/run-now`.
...
`services/*.ts`, with the FE's own URLs, params and payload shapes, as the actor that really makes
it (a registered customer, an approved partner, the seed admin). 80/81 checks passed; the failures
it found are fixed and noted above. To repeat it:
1. Bring the stack up (`docker compose up -d` in the backend repo). If requests hang or `/auth/login`
   500s, check `docker ps -a` — the `petbooker-sql` container has crashed before; `docker start
   petbooker-sql` and wait for `healthy`.
2. Register + confirm throwaway accounts over the API: `POST /auth/register`, then scrape the code
   from MailHog (`http://localhost:8025/api/v2/search?kind=to&query=<email>`) and
   `POST /auth/confirm-email`. Re-login after an admin approves a provider — `serviceProviderId`
   only lands in the JWT on the next login.
3. Retry `429`s (the auth endpoints are rate-limited), and delete what you create: bookings and
   reviews first, then services, providers, pets, payment methods, users.

Two traps worth knowing when writing such a script: an unknown **query param** binds to nothing
rather than 400ing (so a typo'd filter silently returns everything), and an explicit `null` on a
non-nullable column gives a **500 with an empty body** where omitting the key succeeds.
...
- The whole stack is gated on `isLoggedIn`; while `isLoading` (session restore) — and again while the deep link resolves — a full-screen `PetLoader` splash is shown.
- `MainTabs` registers all five tabs (`Home`, `Search`, `PartnerHub`, `AdminDashboard`, `Profile`) and renders the custom `components/shared/TabBar.tsx` as the navigator's **`tabBar`** (the built-in one is measured at zero via `tabBarStyle: { display: 'none' }`). It used to be mounted per screen — `footer={<TabBar />}` in each of the five — which stopped working when tab switches became a slide: a bar inside a scene slides away with it. Mounted on the navigator there is exactly one, it sits above the scenes, and it owns its own bottom safe-area inset (it no longer has a screen's `SafeAreaView` above it). It still positions itself `absolute bottom-0`, so it overlays content and costs the scenes no height.
...
- **Tab switches slide horizontally, in the direction the tab sits in the bar** (`navigation/tabTransition.ts` → `useTabSlideOptions()`, applied in `MainTabs`): Search → Home brings Home in from the left, Home → Search brings Search in from the right. Replaced the hard cut that stood from 2026-08-24 to 2026-09-14.
  - **The travel distance is the whole rule.** The named presets (`'fade'`, `'shift'`) cross-*fade* the scenes, and since the scenes are siblings neither can be drawn opaquely over the other — mid-switch both are partly transparent and each screen's card borders, shadows and section outlines ghost through the other. A full scene-width slide with no opacity change keeps the two scenes exactly adjacent, so they never overlap and nothing blends. Shorten the travel (`'shift'` moves 50px) and the ghosting is back.
  - **The travel is the window width, on both designs.** A scene is the whole window on a phone and the window minus the sidebar on the web, so this is exact on one and slightly generous on the other — deliberately, because the two errors are not equal: too short puts the scenes back on top of each other (the ghosting), while too long only delays the incoming screen, and `Easing.out` hides the sidebar's worth of overshoot in ~14ms of a 260ms slide. It used to subtract `SIDENAV_WIDTH.desktop`, which was exact until the sidebar became `fit-content` — it is sized by its labels now, so it varies by language, and subtracting a guess can subtract too little. Pinned by `__tests__/responsive/tabTransition.test.ts`, which asserts the travel is never shorter than the window.
  - No `animation` name is passed — naming one would supply that preset's interpolator as the default. `useReducedMotion` falls back to `animation: 'none'`.
...
|---|---|---|
| HomeScreen | `screens/home-screen/containers/HomeScreen.tsx` | **API-wired** — each row is its own backend endpoint (`services/home.ts`): Near You → `getNearMe({lat,lng})`, Most Popular → `getMostPopular()`, Recently Booked → `getRecentlyBooked()`, Special Deals → `getOnSale()`, all fetched in parallel in `useFocusEffect` (each wrapped so one failing row doesn't blank the page; re-runs when location resolves). Each returns the full service read shape, so a card renders from the rail alone — `price` (post-discount), `appliedDiscountType`/`Amount` (the "-25% OFF" badge), `rating`, `imageUrl`, `currency` — with no second request. **Every rail is always drawn, cards or no cards** — a row that deleted itself when it was empty took its heading with it, so the page reflowed between loads and a reader could not tell a row with nothing in it from one this build does not have (on a wide window it silently collapsed from four sections to one). A row with no cards shows a compact strip in their place (`emptyRow`), and **which strip depends on why it is empty** (`emptyFor`): a failed request gets "couldn’t load" + a Retry labelled with its row — which reloads the whole page, since the four rails answer from one host — while a row that simply came back empty gets its own line ("nothing near you yet", "no deals right now", …) and no Retry, because an empty catalogue is not something Retry can fix. Per-row failure is tracked separately (`failed` + `nearYouFailed`) for exactly that reason. **Card tap → ServiceDetail for that specific service** (service-centric — no provider step; the booker reads the full service before BookService); pills match `serviceProviderType` enum labels. There is no greeting/tagline card. |
...
| ServiceDetailScreen | `screens/service-detail-screen/containers/ServiceDetailScreen.tsx` | **API-wired — the pre-booking read-everything step** (route `ServiceDetail`, param `{ service: ServiceDto }`). Home/Search cards now land here first (not straight on BookService). On mount fetches the full service (`getService(id)`) for its `reviews[]` (filtered client-side to `approvalStatus === Approved` — the embed carries all statuses), then **`getServiceProvider(service.serviceProviderId)`**, because a service read carries no provider record at all. Both fail-soft: the route-param service still renders, just without reviews / the provider card. Renders hero image, name/type, rating + effective/struck price, About, provider (name/avatar/address/verified), additional services (`getEnabledServiceAddons`), accepted pets (`details.acceptedSpecies` FLAGS), "Good to Know" facts (duration/weight/live-tracking/lead-time), working hours (`schedulesToWorkingHours`), and the approved reviews. A sticky **Book Now** CTA is the only path forward → `navigate('BookService', { service })` with the full DTO. |
| BookServiceScreen | `screens/book-service-screen/containers/` | **API-wired** — books **one specific service passed in as a route param** (`{ service }`, a `ServiceDto`, from ServiceDetail's Book Now); `serviceProviderId` is read off the service. There is **no "Choose Service" step** — Step 1 shows the fixed service. Add-ons are whatever the provider offers (`getEnabledServiceAddons` over `service.additionalServices` — an open list, not a fixed three); which address pickers appear is derived from the selection's `distanceLeg`s via `requiredAddressesFor()`, and a per-distance extra **requires its location on a map** (`MapAddressPicker`) before continuing — the dropped pin is reverse-geocoded into an `AddressDto` via `services/geocoding.ts` and carried on the appointment as `pickupAddress`/`leaveOverAddress`. **Every amount on the screen comes from `getBookingQuote()`**, re-run (debounced on a serialized `quoteRequest`) whenever something price-relevant changes; while it's in flight or **failed, continuing is blocked** — the screen never shows a stale or guessed price. The appointment handed to ReviewBooking carries the server's own bill lines (`addons: BookingAdditionalServiceReadDto[]`) plus the ids to re-send (`addonIds`) and the measured `pickupDistanceKm`/`leaveOverDistanceKm` for display. Real pets + shared DatePicker for the date (unscheduled weekdays grayed out via `isDateEnabled`). Time is picked via `TimeSlotPicker` (hourly 1h slots **derived from `service.schedules`** per weekday — a day with no schedule shows no slots; availability also factors the provider's real bookings for that day; see `services/bookings.ts` notes). Passes `{ service, appointments }` to ReviewBooking. |
| ReviewBookingScreen | `screens/review-booking-screen/` | **API-wired** — takes `{ service, appointments }`; shows the **service** (name/type/image), not a provider. The `PriceBreakdown` lists Service, discount, and each add-on; **per-distance add-ons render an itemized sub-breakdown** — Start fee / Distance ({km} × {rate}) — from the server's own line (`baseFee`/`perKmFee`/`distanceKm` on `appointment.addons[]`, quoted in BookService), aggregated by name across appointments (base fee shows `×count`). The per-km portion is derived as `price − baseFee` so the sub-lines always reconcile to what is actually charged rather than re-deriving the surcharge formula. Confirm resolves a payment method (auto-creates a default if none) and POSTs a real booking via `createBooking()` (`serviceProviderId` from the service). Payment selector is still UI-only (online/cash → Card/Cash). |
...
| BecomePartnerScreen | `screens/become-partner-screen/containers/` | Partner signup pitch. The stats card above "Why Partner with Us" is **API-wired** — `getPlatformStats()` on mount replaces what were three hardcoded marketing figures (`10K+` / `2K` / `4.8★`). Each tile is dropped when its backing count is 0 (`totalReviews`, `earningPartners`), and a failed fetch hides the card rather than showing numbers the platform can't back — deliberately fail-soft, since the pitch below it is the primary content. |
| PartnerApplicationScreen | `screens/partner-application-screen/containers/` | Multi-step partner application form. **Step 2's service picker is the `serviceProviderType` enum** (`useEnums()`, labelled via `tEnum`, static `PROVIDER_TYPE_LABELS` as the pre-load fallback) and is a **single choice** — a ServiceProvider has one `type`. It was a local list of six invented ids (`dog-walking`, `training`, `veterinary`) that matched no enum member, and `createServiceProvider` never read it: every application was filed as `type: 0` (Sitter). The type is now required (blocked at step 2 and again at submit — never defaulted) and sent on the payload as `serviceType`. AdminAddPartnerScreen reuses the same steps, so it shares the form shape. |
| ApplicationSubmittedScreen | `screens/application-submitted-screen/containers/` | Post-application confirmation |
| PartnerWelcomeScreen | `screens/partner-welcome-screen/containers/` | The "You're Approved!" celebration + five-slide partner tour. **Opened by the unread `ServiceProviderApproved` notification** — at sign-in (`App.tsx` asks for one, marks it read, and navigates) or by tapping it in the inbox (`navigation/notificationRoute.ts`). It is deliberately NOT a per-device "seen" flag any more: that is a fact about the browser, so a partner approved months ago met the tour again on every fresh browser or reinstall, between them and the app they had signed in to use. On the web design `AppShell` unmounts the sidebar and top bar for this route (`FULL_BLEED_ROUTES`), so the celebration owns the whole window instead of sitting in the content column. Terminal: the CTA and Skip both reset onto the PartnerHub tab. |
...
| MyServicesScreen | `screens/my-services-screen/containers/` | Partner's listed services. Add/Edit Service (`AddEditServiceScreen`) lets the partner tap a service image to pick the **profile photo** (`isSelected`, round-trips in edit). The **"Working Hours"** and duration-tier sections persist **nested on the service write** (edit prefills from `service.schedules[]` / `service.pricingOptions[]`); the only follow-up call left is `clearServiceSchedules` when every day is switched off. The **"Service Location"** section sets `service.address`: `MapAddressPicker` pin or a one-tap "Use my profile address" shortcut (copies `getUser(id).address` fields, **never the id** — no row sharing); saved via `resolveServiceAddressForSave` (see services.ts address notes). The **"Additional Services"** section is an **open list** the provider adds rows to (no fixed Pickup/Drop-off/Special-Needs trio): each row picks how it bills (`CHARGE_TYPE_OPTIONS`) and, when per-distance, which journey (`DISTANCE_LEG_OPTIONS`) plus baseFee/perKmFee/free/max km. The provider **doesn't name an extra** — `entryToAdditionalServices` derives it (a per-distance one from its leg, a flat one from the service name) and returns an ARRAY, because a **round-trip row expands into a Pickup and a Drop-off** sharing fees (the API prices each leg against its own measured distance). Row `id`s are preserved so an edit updates in place instead of churning the row and orphaning the bill lines of bookings that reference it; only the first leg of a split can inherit the id. Saved nested on the service write (`uiToServiceDto` → the desired full set) |
| ServicePreviewScreen | `screens/service-preview-screen/` | Preview service before publish |
| ConversationsScreen | `screens/messages-screen/containers/ConversationsScreen.tsx` | **API-wired — the message inbox** (route `Messages`). The load is `useConversationsInbox()`: `getConversations` on focus, the live `ChatInboxUpdated` row spliced to the front rather than a refetch, and pull-to-refresh. **Phone** — the list, and tapping a row pushes `Chat` over it. **Web** — `MessagesSplitView`: the list is the left column of a two-pane messenger with a thread open beside it, the most recent preselected, and clicking a row swaps the thread in place (no navigation, so the list the user is choosing from stays on screen). |
| ChatScreen | `screens/messages-screen/containers/ChatScreen.tsx` | **API-wired — one conversation** (route `Chat`, params `ChatThreadTarget`: a `conversationId`, a `serviceProviderId`, or a `bookingId` — the last two get-or-create). The screen only picks the frame: the bare thread on a phone, the same `MessagesSplitView` on web with this thread open in it, so a thread reached from a notification or a shared URL looks like one reached by clicking. Selecting another thread there rewrites the URL with `setParams` rather than pushing, so browser Back still leaves Messages. |
| ChatThread (component) | `screens/messages-screen/components/ChatThread.tsx` | The thread itself — header, keyset-paged history, live hub subscription, typing, read ticks, composer (locked per `conversation.access`). `embedded` is the split view's frame: no back arrow, fills the pane instead of pinning to the viewport. The messages sit on a brand-green → white diagonal gradient (`THREAD_GROUND`, `expo-linear-gradient`; green top-left under the other side's white bubbles, white bottom-right under your green ones), on the phone design too; it replaced the pet pattern, which competed with the bubbles. |
...
| NotificationSettingsScreen | `screens/notifications-screen/containers/NotificationSettingsScreen.tsx` | **API-wired — notification PREFERENCES** (route `NotificationSettings`, header "Notification Settings"). Formerly the `Notifications` screen; renamed/moved here. `getNotificationSettings` on focus; each toggle persists via `saveNotificationSettings` (POST/PUT). **The push row is wired to the DEVICE** (`usePushPermission`): it reads as ON only when the account preference *and* the OS permission agree, turning it on raises the real OS prompt and records nothing if refused, turning it off also retires this handset's token, and a blocked permission shows `PushPermissionNotice` with an Open Settings button that deep-links to the OS (`Linking.openSettings`). Web and simulators render the row disabled with an "isn't available" notice instead of a switch that could never work. **Quiet hours are editable** — Start/End rows open the shared `TimePicker` and persist `dndStartTime`/`dndEndTime` (the backend suppresses push inside that window via `NotificationQuietHours`); this replaced a "Customize Schedule" button that had no `onPress` at all. The old id/userId-as-0 write bug is fixed backend-side (BACKEND_GAPS N1 resolved 2026-07-16): POST returns real ids (id = userId = the row's PK) and the create→update flow works. Reached from the Profile "Notifications settings" menu item. |
| NewRequestsScreen | `screens/new-requests-screen/containers/` | **API-wired** — partner's bookings via `getBookings({ serviceProviderId })`; New/Accepted/Declined from state+currentStatus; accept → `confirmBooking` (POST /bookings/{id}/confirm), decline → a reason-input modal → `declineBooking(id, reason)` (POST /bookings/{id}/decline; reason stored as `cancelReason`, blank falls back to a generic reason); client name/email/avatar from the booking's populated `user` include; each card lists the booker's selected add-ons by name + charged price, read straight off the booking's `additionalServices[]` bill lines (no re-deriving from flags, no recomputing — the server froze both when it priced the booking) |
...
| `ScreenLayout` | `headerVariant?`, `headerTitle?`, `showBackButton?`, `onBackPress?`, `children`, `footer?`, `contentRounded?`, `safeAreaBg?`, `contentBg?` — **web-only, all inert on mobile:** `width?`, `webHeaderRight?`, `webBare?` | The root of every screen, in **both designs**. Mobile: green `AppHeader` (which owns the top inset) + rounded content sheet, which **must** stay opaque — it is pulled 32px up over the header and is what hides scrolled content behind it — and which reserves the bottom inset when the screen has no `footer` **and is not inside the tab navigator** (there the `TabBar` owns it). There is deliberately no `SafeAreaView`: see Safe areas. Web: `PageHeader` + width-capped `ContentContainer`, no safe area, no coloured slab — and `showNotificationButton` is **ignored**, because `TopBar` owns the bell. |
| `TabBar` | `state?`, `navigation?` (both supplied by the navigator) | Bottom tab bar of the **mobile design** — destinations from `navigation/navItems.ts`, role-gated via `useAuth()`. **Renders `null` above 768px**, where `SideNav` is the navigation. Mounted once as the tab navigator's `tabBar`, not by each screen — see the App.tsx notes. Reserves the bottom safe-area inset itself; content clears it with `useTabBarSpacing()`, never a hardcoded number. |
...
| `TwoColumn` | `aside`, `asideWidth?`, `gap?`, `asideFirstOnMobile?`, `asidePosition?` ('left'\|'right') | Main content beside a sticky side panel on web; stacked on mobile. The shape most detail/form screens want on a wide window. Used **inside** a screen's body, not as a `ScreenLayout` prop — on mobile the panel has to live inside the screen's own ScrollView. `asidePosition='left'` is for a panel that *controls* the content rather than summarising it — the search filter rail. |
...
| `ServiceResultRow` | `service`, `image`, `typeLabel`, `distance?`, `badge?`, `onPress` | **Web design only** — one search result as a full-width row: media / substance / decision. Carries what actually decides between two providers (description, accepted species, extras, live tracking, duration tiers, the struck-through pre-promotion price, the deposit due now) in place, instead of making the reader open each result. The phone keeps `ListView`'s compact card — at 390px these three regions each become a full-bleed block and one result fills a screenful. See the browse-list note under Key Conventions. |
| `SearchFilters` | `value`, `onChange`, `maxPrice`, `availableAddOns?` | The filter controls with no chrome, fully controlled. Rendered in **two** places: the sticky rail beside the web results (applies on change) and inside `FilterModal` on a phone (held as a draft behind Apply). One copy is what stops the two offering different filters — which, now every filter is a query parameter, would be two different searches. Also exports `FilterState`, `EMPTY_FILTERS` and `activeFilterCount`. |
...
design, for signed-out users, and for the routes in **`FULL_BLEED_ROUTES`** — today just the
partner celebration, which is a moment rather than a page and reads as a green panel in the
corner of the app when the chrome frames it. Unmounting the chrome is how a screen gets the whole
window; `position: fixed` is not, because a fixed box is positioned against the nearest
transformed ancestor and the navigator puts transforms on its scenes whenever anything animates.
...
`TabBar` **renders `null` above 768px**. It is mounted once, as the tab navigator's `tabBar`
(see the App.tsx notes above) — not by the screens, which pass no footer.
...
- **`hex`** — raw hex object (`bg`, `card`, `text`, `subtext`, `border`, `inputBg`, plus `mutedIcon` for a large empty-state glyph and `chipBg` for an inactive count badge) for `style={}` props / native components that can't take Tailwind classes (admin screens, pickers, maps).
...
### Paged lists, and where filtering belongs
Any list long enough to page uses **`usePagedList`** (accumulated rows, first-load vs appending, stale-response and double-fire guards) with **`LoadMoreFooter`** + `isNearBottom` on the ScrollView's `onScroll` — it loads the next page as the reader nears the bottom, with the footer button as the manual fallback. These screens are `ScrollView` + `.map()`, not `FlatList`, so there is no `onEndReached`.

**A filter on a paged list is a query parameter, never a `.filter()`.** The client only holds the pages that have arrived, so a client-side predicate narrows *those* rather than the search: a narrow filter shows a handful of matches out of page one, the "N found" count describes the loaded subset rather than the result set, and scrolling keeps pulling rows that are then thrown away. Sorting has the same problem and one more — the server's `Skip`/`Take` needs a **total** order (every sort ends on the row id) or a row can appear on two pages and another on none.

The one exception is a list that arrives **complete**: the Home rails return a single capped list with no paging, so narrowing them in the client filters the whole result set and is sound. `SearchScreen` distinguishes the two explicitly (`clientFiltered`) rather than leaving it to chance.

Filter *options* have the mirror-image trap: derive them from the current results and they collapse as you filter (filter by "Pickup" and every row offers Pickup, so that is the only chip left and there is no way back). They are facts about the catalogue, so sample them once — `SearchScreen`'s `facets` does, and gets an exact price ceiling by asking for one row sorted most-expensive-first.
...
- **Never build an error string from a response body by hand — `parseApiError` owns every shape the API returns** (validation envelope, ModelState, `{ message }`/`{ detail }`/`{ title }`/`{ error }`, plain text). See the HTTP Layer section above for the resolution order and the field-labelling rule.
- The `fallback` you pass is what the user sees when the API says nothing usable, so make it name the action ("Couldn't cancel this booking.").
...
### Money & Currency — `services/currency.ts` is the single source of truth
- **Never write a currency symbol into JSX, a data constant, or a translation string.** A hardcoded `$` was wrong for every provider not pricing in dollars (it rendered 100 dinars as "$100"); a hardcoded `€` prefix was wrong for every currency whose symbol belongs *after* the number. Three i18n templates had a `$` baked in (`'${value} Off — {name}'`) — templates now take an already-formatted amount.
- **`services/currency.ts` is the only import path.** Don't re-export `formatMoney` from another module: a second equally-valid path is how it got buried in `services/bookings.ts` and re-invented as 17 hardcoded `$`s in the first place.
- A currency here is a symbol **plus its placement**: `USD`/`GBP` → prefix, tight (`$52`); `EUR` → suffix + space (`52 €`); `RSD` → `52 RSD`; `RUB` → `52 ₽`; an unknown code → `52 CHF`. Amounts round to ≤2 decimals with trailing zeros trimmed.
- **Display** → `formatMoney(amount, currency)`. Pass the currency **that came with the data** (`ServiceDto.currency` via `serviceCurrency(svc)`, `ServiceDiscountDto.currency`, `BookingReadDto.priceCurrency`, `BookingQuote.priceCurrency`, a stats block's `currency`); omit it only for amounts the API doesn't stamp.
- **In a component** → `useCurrency(dataCurrency?)` → `{ code, money, perUnit, wrap, prefix, suffix, symbol, format }`. `money(n)` formats, `perUnit(n,'km')` gives `2 €/km`, `wrap('2K')` attaches the symbol to an already-abbreviated figure.
- **Price INPUTS** → `components/shared/CurrencyInput.tsx`, which puts the symbol on the conventional side of the `TextInput` (prefix currencies before it, suffix currencies after). Container/input/affix classes stay caller-controlled so a field's look doesn't change. For inputs that already have their own affix (the promo `%`), use `useCurrency().prefix` / `.suffix` directly.
- **Only money converts.** Ratings, counts, distances, durations and percentages are never currency-formatted.
- **Fallback chain**: the amount's own currency → the user's `preferredCurrency` (mirrored into the module by `AuthContext` via `registerDisplayCurrency`, the same pattern as `registerApiLanguage`) → `DEFAULT_CURRENCY` (`RSD`). Changing it in Settings calls `refreshUser()` so mounted screens re-render. Note this falls back to the *display preference*, not `BASE_CURRENCY`: an amount the server sent has already been converted into the caller's preference, so an RSD fallback would mislabel a converted EUR number. Use **`BASE_CURRENCY`** explicitly only where the UI means "what this will be charged as".
- **`ServiceDto.currency` is on the wire on every service read, the Home rails included (BACKEND_GAPS S8 resolved; re-verified live 2026-08-10)** — read it via `serviceCurrency(svc)` (`services/services.ts`), which returns `undefined` when a record doesn't say so the preference fallback still kicks in.
- **The API converts money BOTH WAYS, and a write must DECLARE its currency.** Storage is always RSD, but a GET converts every amount into the caller's `preferredCurrency` and stamps the code it used. A **write** converts *from* whatever `currency` the payload declares — and **omitting it means RSD**. So a form prefilled from a read holds display-currency numbers, and sending them back undeclared silently rescales them: verified live, a EUR viewer opening a service and saving it **unchanged** stored `0`. `services/currency.ts` exports **`declaredWriteCurrency(dataCurrency?)`** (the record's own code, else the display preference — both resolve from the same `preferredCurrency`, so they agree), and the write DTOs that carry a `currency` stamp it **in the service layer, not at call sites**: `createService`/`updateService` (the root governs the whole aggregate — nested `additionalServices`/`pricingOptions`/`discounts` must NOT declare their own), `create/updateServicePricingOption`, and `create/updateServiceDiscount` (only a **Fixed** discount's amount is money; a Percent one is left alone). A forgotten declaration doesn't fail loudly — it corrupts prices — so keep it in the service function.
...
- **Discounts**: `DiscountType` is `0=Percent, 1=Fixed`, but `PercentAmount` **takes precedence over `Type`/`Amount`** server-side — prefer `percentAmount` when present, whatever `type` says (`formatOfferAmount` does).
- `SUPPORTED_CURRENCIES` (`RSD`/`EUR`/`USD`) is the user's *display* preference list (see `notifications.ts` `preferredCurrency`); payments are always RSD for now — **there is no conversion**, only relabelling.

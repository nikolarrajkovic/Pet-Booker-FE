...
- **Two designs, and the difference is structural, not cosmetic.** `ConversationsScreen` and `ChatScreen` are both *pickers*: below 1024px (`WORKSPACE_MIN_WIDTH`, and always on native) the inbox is a page and a thread is a page pushed on top of it — the shipped phone flow, untouched. At or above it both routes render **`MessagesWorkspace`**: the inbox pinned in a 360px left column with the open thread beside it, threads switching like tabs. Switching is **local state, not a `navigate`** — the inbox is not refetched and the list keeps its hub subscription — while the address bar keeps up regardless: `/messages/:conversationId` opens a thread on the way *in* (so deep links, notification taps and "Message provider" all land somewhere you can carry on from), and the Chat route's own params are rewritten with `setParams` as threads are switched, so a refresh or a copied link lands on the conversation being read rather than the one it was entered with. `setParams` replaces the history entry instead of adding one, so browser Back still leaves the inbox rather than stepping back through every thread read. The pane is the same `ChatThread` the phone renders, passed `embedded` (the pane owns the height, the gutters and the reading-width cap, and there is no back arrow — the list is beside it). Both designs read one `useConversationInbox()` and render one `ConversationList`, so "what does an empty inbox say" and "which row is unread" are answered once. Each design is a separate component rather than branches inside one, so dragging a window across the breakpoint mounts the other cleanly instead of changing a component's hook count.
- **With the list pinned beside the thread, three kinds of staleness became visible** that the phone design hid (there the inbox is a screen away and reloads on focus). All three live in `useConversationInbox` and are pinned by `__tests__/useConversationInbox.test.ts`: (1) the hub's inbox ping is the **recipient's** row, so it carries an unread count — suppressed for the thread currently open (`openThreadId`), or the row you are reading lights a pill that only a reload clears; (2) the ping is **recipient-only**, so a user's own sends never come back — the pane reports them through `onMessageSent` → `noteSentMessage`, or the row beside the composer keeps showing the other side's older line; (3) a thread reached from a service page or a booking is get-or-created server-side and can be **younger than the list**, so `onConversationLoaded` → `noteOpenedThread` upserts it rather than only marking it read. `markThreadRead` returns the previous array unchanged when there is nothing to clear — a new array identity from a no-op re-renders every consumer and loops anything reacting to the list.
- **Mounted is not the same as visible, and `ChatThread` now distinguishes them.** A thread stays mounted behind whatever is pushed on top of it (a booking opened from it on the phone, a second workspace on the web), and two things used to be keyed on mount instead: the `claimActiveConversation` claim, which went on suppressing that thread's toast so the one notification saying "they replied" never arrived; and the read receipt on an arriving message, which wiped the unread badge for a message the user had not seen (the server really did report 0 unread for it). Both are keyed on **focus** now — `useFocusEffect` claims and releases, arriving messages are reported read only while `isFocusedRef` is set, and returning to the thread marks it read up to the newest line it holds. All read receipts go through one `markRead` with a high-water mark, so refocusing does not re-send one.
- **Enter sends, Shift+Enter starts a new line** — `MessageComposer`'s `sendOnEnter`, which `ChatThread` turns on for the web design only (`isWebLayout`, never a native build): on a phone the return key is how you write a second line and there is no modifier to get it back. It is wired to `onKeyPress`, **not** `onSubmitEditing`, which a `multiline` field never fires — which is why the composer had no keyboard send at all.
...
- Per type: request → `NewRequests` (the screen with Accept/Decline, not the read-only recap); live tracking → `LiveSession`; service completed → the inbox, which auto-opens `ReviewModal` (the inbox passes `{ reviewHandled: true }` so an already-reviewed booking falls through to `BookingDetails` instead of a tap that does nothing); review approved/received → `ServiceDetail` for the service the payload names; review declined and the provider/certificate decisions → the feed and the Partner hub respectively, neither having a screen of its own; message → `Chat`; **everything else → `BookingDetails`**, which is also the fallback for a type this build predates, so a notification added to the backend later still opens its booking rather than being swallowed.

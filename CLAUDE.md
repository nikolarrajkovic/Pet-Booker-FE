...
- **Two designs, and the difference is structural, not cosmetic.** `ConversationsScreen` and `ChatScreen` are both *pickers*: below 1024px (`WORKSPACE_MIN_WIDTH`, and always on native) the inbox is a page and a thread is a page pushed on top of it — the shipped phone flow, untouched. At or above it both routes render **`MessagesWorkspace`**: the inbox pinned in a 360px left column with the open thread beside it, threads switching like tabs. Switching is **local state, not a `navigate`** — the inbox is not refetched and the list keeps its hub subscription — while a URL still opens a thread on the way *in* (`/messages/:conversationId` renders the workspace with it selected), so deep links, notification taps and "Message provider" all land somewhere you can carry on from. The pane is the same `ChatThread` the phone renders, passed `embedded` (the pane owns the height, the gutters and the reading-width cap, and there is no back arrow — the list is beside it). Both designs read one `useConversationInbox()` and render one `ConversationList`, so "what does an empty inbox say" and "which row is unread" are answered once. Each design is a separate component rather than branches inside one, so dragging a window across the breakpoint mounts the other cleanly instead of changing a component's hook count.

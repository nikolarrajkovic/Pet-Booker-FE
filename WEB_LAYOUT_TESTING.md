...
| `tabTransition.test.ts` | The tab slide travels **exactly one scene-width** — window width on the phone design, window minus the sidebar on the web one. Shorter travel makes the two scenes overlap, which is the cross-fade ghosting the slide exists to avoid. |

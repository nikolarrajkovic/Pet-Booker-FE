...
| `tabTransition.test.ts` | The tab slide travels **at least one scene-width** — the window width, which is exact on the phone design and slightly generous on the web one (a scene is the window minus the `fit-content` sidebar). Shorter travel makes the two scenes overlap, which is the cross-fade ghosting the slide exists to avoid; longer only costs a few ms behind the easing. |
...
| B19 | Each admin moderation tab asks the server for its own status and order, a picked order stays on its tab, the badges are server counts, and the phone pages the same queue as cards (`AdminModerationLists.test.tsx`) |

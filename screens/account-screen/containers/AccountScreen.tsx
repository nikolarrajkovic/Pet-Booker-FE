  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled) Alert.alert('Could not load profile', e?.message ?? 'Please try again.');
      } finally {
  ...
  const pickProfilePhoto = async () => {
    ...
    if (!result.canceled && result.assets[0]) {
      ...
      const uri = asset.base64
        ...
        : asset.uri;
      setNewPhoto({ uri, fileName: asset.fileName ?? undefined, mimeType: asset.mimeType ?? undefined });
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ paddingBottom: 120 }}>
        ...
        <View className="items-center py-8">
          <View className="relative">
            {avatarUri ? (
              <Image source={{ uri: avatarUri }} className="w-32 h-32 rounded-full" onError={() => setAvatarError(true)} />
            ) : (
              <View className={`w-32 h-32 rounded-full ${isDarkMode ? 'bg-[#243447]' : 'bg-brand-100'} items-center justify-center`}>
        ...
        <View className="px-6">
          ...
          <View className="mb-6">
            ...
            >
              ...
              <Text className={`ml-3 flex-1 ${currentAddress ? inputText : subtextColor}`} numberOfLines={2}>

...
/**
 * What identifies a thread. Either an existing one by `conversationId` (inbox, deep link), or
...
export type ChatThreadProps = {
  /** Which thread to show. All three forms are get-or-create — see `ChatThreadParams`. */
  params: ChatThreadParams;
  /**
   * Rendered as the right-hand pane of the web workspace rather than as a screen of its own.
   *
   * An embedded thread fills the pane it is given: no viewport-height root, no reading-width cap
   * and no safe-area padding (all three belong to the workspace around it), and no back arrow —
   * the list it would go back to is the column on its left.
   */
  embedded?: boolean;
  /** Back affordance for the standalone screen. Omitted when embedded. */
  onBack?: () => void;
  /**
   * The resolved thread, for a parent keeping a list in step with it — the workspace uses it to
   * highlight the row and drop its unread pill, since `conversationId` is only known after a
   * get-or-create by provider or booking.
   */
  onConversationLoaded?: (conversation: ConversationDto) => void;
  /**
   * A message this pane just sent. The hub's inbox ping is recipient-only, so a parent showing the
   * thread's row beside it has no other way to learn that its last line changed.
   */
...
export default function ChatThread({
  ...
  /**
   * Held in a ref, not a dependency: the parent re-renders when it hears about the thread, so a
   * callback in the effect's deps would re-run the load, which would call back again — a fetch
   * loop that only shows up once the workspace is wired to it.
   */
  const onLoadedRef = useRef(onConversationLoaded);
  ...
  // thread sits under the page rather than hugging the sidebar. Embedded, the workspace's pane is
  // already that column — capping again would strand the thread inside its own half of the page.
  const column =
  ...
  const root =
    isWebLayout && !embedded
      ...
      : // Embedded: the pane is already a definite height, so the thread just fills it. `minHeight`
        // is what keeps the message list — not the pane — the thing that overflows.
  ...
  return (
    <View
      style={[
        ...
        isWebLayout || embedded ? undefined : { paddingTop: topInset, paddingBottom: bottomInset },
        column,
      ]}>
      ...
      <View className={`flex-row items-center border-b px-3 py-2.5 ${borderColor} ${cardBg}`}>
        {/* Embedded there is nothing above this pane to go back to — the inbox is beside it. */}
        {!!onBack && (
          <TouchableOpacity
            onPress={onBack}
            activeOpacity={0.7}
      ...
      <KeyboardAvoidingView style={{ flex: 1, minHeight: 0 }} behavior="padding">
        <ScrollView
          ...
          contentContainerStyle={{
            ...
            paddingVertical: 12,
            flexGrow: 1,
            // A short thread belongs at the FOOT of a tall pane, above the composer, the way every
            // chat client sits: the web pane is most of a 900px window, so top-aligned it left
            // four bubbles stranded at the ceiling with a screenful of empty ground under them.
            // Only with messages to place — the loading, error and empty states stay centred.
            justifyContent: embedded && messages.length > 0 ? 'flex-end' : undefined,
          }}
        ...
        {!loadError &&
          (lockedMessage ? (
            ...
            <MessageComposer
              ...
              sending={sending}
              isDarkMode={isDarkMode}
              // Only on the web design, which is the one with a hardware keyboard: a phone's
              // return key belongs to the message being written, not to sending it.
              sendOnEnter={isWebLayout}
              onTypingChange={
                conversationId != null

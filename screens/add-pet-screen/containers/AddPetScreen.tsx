  ...
  const [petPhotos, setPetPhotos] = useState<{ uri: string; fileName?: string }[]>(() => {
    if (existingPet?.photos?.length) {
      ...
      return existingPet.photos.map((p) => ({
        uri:
          p.src.startsWith('http://') || p.src.startsWith('https://')
            ? p.src
    ...
    <ScreenLayout
      ...
      <ScrollView
        ...
        <View className="mb-4">
          ...
          />
          {errors.breed ? <Text className="mt-1 text-xs text-red-500">{errors.breed}</Text> : null}
        </View>
        ...
        <View className="mb-4">
          <Text className={`text-sm font-semibold ${textColor} mb-2`}>
            {t('addPet.additionalNotes')}
          </Text>
          <TextInput
            placeholder={t('addPet.additionalPlaceholder')}
            placeholderTextColor={placeholderColor}

...
export function PartnerCard({ partner, isDarkMode, cardBg, textColor, subTextColor, borderColor, onPress }: Props) {
    ...
    >
      ...
      <View style={{ flex: 1 }}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 3 }}>
          <Text style={{ color: textColor, fontSize: 15, fontWeight: '700', flex: 1, marginRight: 8 }} numberOfLines={1}>
            {partner.name}
          </Text>
          <View style={{ backgroundColor: cfg.bg, borderRadius: 8, paddingHorizontal: 8, paddingVertical: 2, borderWidth: 1, borderColor: cfg.borderColor, flexDirection: 'row', alignItems: 'center' }}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
          ...
          <Text style={{ color: textColor, fontSize: 12, fontWeight: '600', marginLeft: 3 }}>{partner.rating.toFixed(1)}</Text>
          <Text style={{ color: subTextColor, fontSize: 12, marginLeft: 2 }}>({partner.reviews})</Text>
          <Text style={{ color: subTextColor, fontSize: 12, marginHorizontal: 6 }}>•</Text>
          <Text style={{ color: subTextColor, fontSize: 12 }}>{partner.totalServices} services</Text>
        </View>

        {/* Tags row */}
        <View style={{ flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', gap: 6 }}>
          {partner.services.slice(0, 1).map((svc) => (
            <View key={svc} style={{ backgroundColor: isDarkMode ? '#1e3a2f' : '#E8F5EF', borderRadius: 20, paddingHorizontal: 8, paddingVertical: 2 }}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            ...
            <Text style={{ color: subTextColor, fontSize: 11, marginLeft: 2 }}>{partner.distance}</Text>
          </View>
          <Text style={{ color: subTextColor, fontSize: 11 }}>•</Text>
          <Text style={{ color: subTextColor, fontSize: 11 }}>Joined {partner.joinedDate}</Text>
        </View>
      </View>
    </TouchableOpacity>
  );

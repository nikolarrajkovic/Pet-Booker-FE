    ...
    <View className={`${cardBg} rounded-2xl mb-4 border ${borderColor} overflow-hidden`}>
      ...
      {request.petSpecialNeeds && (
        <View className={`mx-4 mb-3 ${specialNeedsBg} rounded-xl p-3 flex-row items-center`}>
          ...
          <Text className="text-yellow-700 text-xs font-medium flex-1">
            Special Needs: {request.petSpecialNeeds}
          </Text>
        </View>
      )}
        ...
        <View className="mx-4 mb-3">
          ...
          <View className="flex-row flex-wrap gap-2">
            {request.additionalServices.map((svc) => (
              <View
                key={svc}
                className={`px-3 py-1 rounded-full border ${borderColor} ${isDarkMode ? 'bg-[#243447]' : 'bg-white'}`}
              >
      ...
      {request.status === 'new' && (
        <View className={`flex-row border-t ${borderColor}`}>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.7}
            className="flex-1 py-4 flex-row items-center justify-center bg-brand-500 rounded-br-2xl"
          >
      ...
      {request.status !== 'new' && (
        <View
          className={`mx-4 mb-4 py-2 rounded-xl items-center ${

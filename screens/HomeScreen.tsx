      ...
      <ScrollView className={`flex-1 -mt-8 ${contentBg} rounded-t-3xl`} contentContainerStyle={{ paddingHorizontal: 24, paddingTop: 32, paddingBottom: 80 }}>
          ...
          <View className="items-center mt-6">
            <ServiceBubble
              ...
              onPress={() => (navigation as any).navigate('Search', { serviceType: 'Dog Sitting' })}
            />
          </View>
          ...
          <View className="flex-row justify-center items-center mt-6 gap-6">
            <ServiceBubble
              label="Boarding"
              bg="bg-brand-500"
              icon={<MaterialCommunityIcons name="home-heart" size={28} color="white" />}
              onPress={() => (navigation as any).navigate('Search', { serviceType: 'Boarding' })}
            />
            <ServiceBubble
              label="Pet Hotels"
              bg="bg-brand-500"
              icon={<MaterialCommunityIcons name="office-building" size={28} color="white" />}
              onPress={() => (navigation as any).navigate('Search', { serviceType: 'Pet Hotels' })}
            />
          </View>
        ...
        {/* Bottom spacing so content isn't hidden by tabs */}
        <View className="h-24" />
      </ScrollView>
